...
//Anuel
    ...
    public void mostrarCurso(DefaultTableModel tab) {
        try {
            s = conexion.createStatement();
            ...
            while (rs.next()) {
                tab.setRowCount(cont + 1);
                tab.setValueAt(rs.getString("name"), cont, 0);
                tab.setValueAt(rs.getString("code"), cont, 1);
                tab.setValueAt(rs.getString("modality"), cont, 2);
                tab.setValueAt(rs.getString("duration"), cont, 3);
                tab.setValueAt(rs.getString("credits"), cont, 4);
                cont++;
            }
        } catch (Exception e) {
        }
